...
colors:
  ...
  # What a glass surface becomes under `prefers-reduced-transparency: reduce`
  # (not painted at rest). Both themes defined.
  ...
  glass-solid-dark: "color-mix(in oklab, var(--white) 88%, var(--ground))"
...
Since 2026-09-05 that glass is tint without blur: every `backdrop-filter` and `-webkit-backdrop-filter` declaration in `app/globals.css` was deleted, and the four-opacity ladder now does its legibility work by flat alpha compositing alone (see The Tint-Is-Legibility Rule). The change makes universal what production Chromium had already been showing by accident, a build optimizer there had long stripped the unprefixed declaration, keeping only the inert `-webkit-` form, so no browser had ever actually rendered blur on that surface in production; only Safari, which kept the working declaration, ever showed it. Parth compared both looks and chose the no-blur version on purpose, for every browser and both themes, rather than fix the pipeline to restore blur everywhere.
...
**Phone-native, not scaled down.** A twelve-PR mobile overhaul (2026-09) rebuilt the phone form of every surface as its own layout rather than a squeezed desktop one: a two-line hero with its own scroll-down cue and a fold-peeking flagship card, work cards that trade the mark-dominant desktop layout for a 60px chip beside a full-width tagline, the side menu that becomes a bottom sheet with a grabber and its own close control, a peek-and-snap slideshow with a native `<dialog>` expand for tall captures, and a single consolidated phone breakpoint at 760px (there is no 720px or 640px any more) with two narrow named exceptions below it. The glass, the caps, the one accent and both themes carry over unchanged; what changes is geometry, density and how a thumb reaches things.
...
- One frosted glass at four opacities as the whole material system; a flat tint carries legibility, not a blur (The Tint-Is-Legibility Rule)
- Exactly one accent colour, split into a shape token and a text token
- A real 128px grain raster, not an SVG filter
- Flat at rest: edges alone convey depth (nothing on the page blurs its backdrop), and the one shadow is drawn only while a card is lifted off the page
...
- Two full themes, both first-class, and both followed by default rather than defaulted to light
- One phone breakpoint (760px), not three: every phone-scoped rule in the system keys off it, with 400px and 360px as the only two narrower named exceptions
...
- **Glass, Solid** (`{colors.glass-solid}`): not a fifth rung of the ladder, and not painted at rest. `color-mix(in oklab, var(--white) 92%, var(--ground))` light, 88%/12% dark. The only thing every glass surface becomes under `prefers-reduced-transparency: reduce` (see The Reduced-Transparency Fallback), so text keeps the ground it was measured against without the wash showing through.
...
- **Display** (700, `clamp(28px, 5.1vw, 64px)`, line-height 1.12, tracking 0.005em, uppercase): the hero headline only, the name. Under 760px the whole title drops to `clamp(28px, 10vw, 44px)` / line-height 1.06, and `.hero__name` switches to `display: block` so the name takes its own line. See The Two-Line Phone Headline Rule.
- **Descriptor** (700, `clamp(14px, 3vw, 32px)`, line-height 1.2, tracking 0.005em, uppercase, display face): the rolling secondary line under the name. See The Name-Then-Descriptor Rule. **Both bounds are px, never rem, and that is a rule rather than a preference**; see The Px-Bound Display Rule. Under 760px it runs `clamp(16px, 5.6vw, 24px)`, 0.56x the phone name size at every width down to 280px.
...
**The Two-Line Phone Headline Rule.** Below 760px the hero title is exactly two lines, never three: `.hero__name` becomes `display: block` so the name takes its own line under "HEY, I'M", and the two-line title box holds at every phone width. Inside that box the name itself carries a hard invariant, verified at every width from 280 to 760px and at both the default root font size and a simulated 200% (20px) root: `PARTH DOSHI` clears **97% of the column** at every one of those widths. The `10vw` fluid term in `clamp(28px, 10vw, 44px)` is the largest slope that holds the invariant; a shallower slope left headroom unused, a steeper one broke it before the 44px cap took over. See `.impeccable/plans/mobile-overhaul/pr-02-hero-and-fold.md` for the per-width table.
...
The hero is its own grid: `min-height: 100svh`, `align-content: center`, `justify-items: start`, `text-align: left`, `padding: 120px var(--gutter) 56px`, `max-width: calc(1140px + var(--gutter) * 2)`, `margin-inline: auto`, `overflow: clip`. Its column is declared `grid-template-columns: minmax(0, 1fr)`, not left implicit, so children have a width to shrink against and the rolling headline can never overflow the viewport.

Under 760px the hero is bottom-set rather than centred (`align-content: end`, `padding-block: 96px 36px`), and its height is `calc(100svh - var(--peek))` where `--peek: 180px` is deliberately short of the full fold: the flagship work card is designed to peek roughly a fifth to a quarter of its own height (about 20-23%) above the bottom edge at load, as a hint that there is more below. `--peek` is derived from the Work section's own geometry (its 20px top padding, the 34px section title, the title's 20px margin, and about 98px of card), not a magic number. A `.hero__cue` scroll-down link (a 44px glass-free hit target, nudged left so its stem lands on the column's edge) sits under the availability line, holding a hero-local `ScrollCue` glyph (2026-09-10) rather than the shared `ArrowDown`: the same vertical sine the name waves on, turned into a short squiggle with a chevron tip, so the one boilerplate mark on the page carries the site's own signature instead of a stock icon while the chevron still reads unambiguously as "down." A small reduced-motion-gated nod animation plays under it.

Work is a two-column grid (`repeat(2, minmax(0, 1fr))`, 18px gap) with the flagship spanning `1 / -1`; it collapses to one column at **1000px**, not 760 (since 2026-09-03). A chunking break between the flagship's first pair and the rest was tried and reverted the same day (2026-09-10): a full-span spacer big enough to read as intentional grouping (28px, on top of the 18px gap either side of it) was also big enough to break the grid's own 18px rhythm everywhere else, and with no border or label to explain it, it read as a spacing bug rather than a device. The grid stays one uniform rhythm. Two 370px cards side by side gave each name and tagline a 155px column; a two-column card needs about 235px of text column to read, which is what 1000px provides. The phone-level card rules (the 60px mark chip, the flagship's 16:9 cover band, the corner cue) stay at 760px, so between the two a single card at full width uses the desktop card layout. `.work__item` is itself `display: grid` so `.card { height: 100% }` resolves and a row's cards share a bottom edge. Experience is a rail-and-column timeline (below). About is `minmax(0, 1fr) minmax(0, 1.5fr)` with a 48px gap, collapsing to one column with a 20px gap at 760px. Contact is left-aligned like everything else, with 96px/128px vertical padding.
...
The header is fixed with `pointer-events: none` and re-enabled on its children, so it never blocks the page underneath; padding `22px 28px`, dropping to `max(14px, --safe-top) max(16px, --safe-right) 14px max(16px, --safe-left)` under 760px (the shell mark drops to 50px tall there too). The scroll ring sits fixed at 28px from the bottom-right corner, `max(18px, --safe-right)` / `max(18px, --safe-bottom + 6px)` under 760px. Every fixed edge on a phone routes through the four `--safe-*` tokens (`env(safe-area-inset-*, 0px)`), and `viewport.viewportFit` is `"cover"` in `app/layout.tsx`, so the page draws under a notch or a home indicator on purpose rather than by accident.

Breakpoints in use, top to bottom: **900px** (the flow diagram turns from a row into a column), **1000px** (the Work grid collapses to one column, and the gradient-edge parity moves with it: `[data-edge-2col]` at 1001px and up, `:nth-child(odd)` at 1000px and down), **760px** (THE phone breakpoint), **400px** and **360px** (named exceptions below it, see below).

**The One Phone Breakpoint Rule.** Before the mobile overhaul the phone system was split across 640px, 720px and 760px, an inconsistency the program's first PR closed: everything that used to key off 640 or 720 (shell padding and mark size, scroll-ring insets, hero type, blob sizing and drift, the outline stroke floor) now keys off **760px** alongside what already lived there (card padding and media share, the flagship stacking, section padding, gutter, About and timeline collapse, case-study spread, tech tile size, scroll-motion cutoff at 761px, content-visibility, reduced-motion phone overrides). There is no 720 and no 640 any more: a phone is one device class and gets one threshold. Two narrower thresholds exist only as named exceptions below it: **400px** (the menu widens toward the sheet's own max-width and `.pill`/`.timeline__project-name` may wrap) and **360px** (the contact email steps down one size). A new phone rule reaches for 760px first; a third exception below it needs a real reason.
...
**The Portrait-vmax Rule.** Blobs are sized in `vmax` on desktop (40 to 46vmax) but in `vw` under 760px (76 to 84vw), because on a phone `vmax` is the viewport *height*, which makes the blobs full-bleed and swallows the body copy.
...
There is no blur anywhere on the page. Every `backdrop-filter` and `-webkit-backdrop-filter` declaration was deleted project-wide on 2026-09-05 (14 rules, one per glass surface, see The Tint-Is-Legibility Rule), so a surface's weight now comes only from which rung of the glass ladder it sits on (`--glass`, `--glass-strong`, `--glass-raised`) plus its 1px `--glass-edge`. The card's media frame still carries no tint of its own: it is inside the card, which has one.
...
**The Blur-Is-Legibility Rule, retired 2026-09-05.** It used to read: any surface that carries text over the wash must carry `backdrop-filter: blur() saturate()`, because the blur was what kept text readable over a saturated blob. That mechanism is gone: every `backdrop-filter` and `-webkit-backdrop-filter` in the stylesheet was deleted the same day, reproducing universally what production Chromium had already been showing by accident, a build optimizer there had long been stripping the unprefixed declaration, leaving only the inert `-webkit-` form, so no browser had ever actually blurred that surface in production; only Safari, which kept the working declaration, ever rendered the blur. Parth compared both looks and chose the no-blur version for every browser and both themes, rather than fix the pipeline to restore blur. The concern the rule protected is still live and still met: re-measured on `.case__back` and `.case__meta`, the rule's own worst-case surface (see The Surface-Not-Spacing Rule), contrast now reads light p20 7.88 / 8.79, dark p20 12.25 / 14.04, both themes clear the 4.5:1 floor by more than 2.5×. The flat tint, not the blur, was carrying that number the whole time; blur only ever softened the wash's texture behind the text.

**The Tint-Is-Legibility Rule**, its successor. Every surface that carries text over the wash still carries a flat, alpha-blended tint: one rung of the glass ladder (`--glass` / `--glass-strong` / `--glass-raised`). That tint is what keeps text readable over a saturated blob; it is still a functional requirement, not a finish. Nothing on the page samples or blurs what sits behind it any more; the ladder itself is unchanged, only the filter that used to sit behind it is gone.
...
**The Opaque-Media Rule**, a named amendment, now to The Tint-Is-Legibility Rule (its parent was The Blur-Is-Legibility Rule until that mechanism was retired 2026-09-05). The case-study screenshot frame is **opaque** (`--media-well`), because its content is opaque: a tint exists to keep *text* legible over the animated wash, a screenshot has no such job, and tinting behind it would be finish rather than function. The rule's second clause used to license the frame to **travel under scroll motion**, reasoning that predates the 2026-09-05 change and is now vacuous, since no surface anywhere carries `backdrop-filter` to re-sample and re-blur its backdrop every frame. Nothing travels now that the screens are a slideshow rather than a pinned band; the clause is kept only as the reason a future travelling surface should stay opaque first, not because it currently distinguishes anything.

**The Surface-Not-Spacing Rule.** Text that sits outside the calm band cannot be rescued by moving it. The band is only about 270px tall and moves with viewport size (the blobs are `vmax`-sized), and the case-study header block is taller than the band anyway. Text pinned to a page extreme gets a surface; padding tuned to clear a blob at one width is wrong at every other. The case-study header takes `--glass-strong`, the lit rung above a card's `--glass`, and runs its metadata line in `--ink` rather than `--ink-2` to hold the floor. The hero's status line used to take a `--glass-raised` pill for the same reason; the 2026-09-03 declutter deleted the line rather than the surface, and what replaced it sits inside the band on no surface at all. The rule is unchanged: the cheapest fix for text outside the band is still to not put text there.
...
**The Reduced-Transparency Fallback.** The glass ladder is a tint, not a blur, but a flat 40%-alpha tint is still translucency in the OS sense: the blobs show through. Under `prefers-reduced-transparency: reduce`, every glass surface on the page (`.card`, `.card__media`, `.timeline__card`, `.timeline__project`, `.timeline__project-mark`, `.flow__node`, `.flow__branch`, `.flow__bus`, `.case__headline`, `.menu`, `.pill`, `.live-link`, `.card__repo`, `.tech__item`, `.shell__mark`, `.theme-toggle`, `.scroll-ring`, `.shots__arrow`) repaints its background to `--glass-solid`, a near-opaque mix of `--white` and `--ground` (92%/8% light, 88%/12% dark) so text keeps the ground it was measured against without touching `backdrop-filter`, which no rule in the sheet sets any more. The screenshot well is already opaque and is untouched; the preference is about surfaces over content, not about the wash itself.

**The Forced-Colors Fallback.** The timeline rail and the flow diagram's connector row are drawn as gradients (`.timeline__list::before`, `.flow__steps::before`), and a gradient is stripped entirely under `forced-colors: active` with nothing replacing it by default. Both rules get an explicit `background: CanvasText` inside `@media (forced-colors: active)` so the sequence they draw survives; every other gradient on the page (the card edge, the live-link ring, the specular highlights) is decorative rather than sequence-bearing and is allowed to disappear under forced colours.

**The Per-Selector Intrinsic Size Rule.** Below-the-fold sections get `content-visibility: auto` under 760px so the browser skips layout and paint for what is off-screen, but each selector carries its own `contain-intrinsic-size` guess rather than one shared value: `.section--exp` 1000px, `#about` 800px, `.contact` 550px, `.case__section` 500px, `.shots` 400px, `.case__foot` 420px. A single shared guess (600px was tried) is wrong for most of these sections and, worse, an undershooting guess compounds down the page: a browser's one-time scroll-to-fragment on load runs before an undershooting sibling above the target has unskipped to its real height, so `/#about` and `/#contact` landed short. Six selectors, six guesses, sized to what each section actually is.
...
The recurring silhouettes are the circle, the pill and the blob. The circle appears as the scroll ring, the live-link dot (9px with a 3px halo), the card's repo button (64px in its corner variant, 56px beside the live pill), the timeline node (14px, 2.5px ink border, ground fill) and the flow diagram's 26px index badge. The pill is the small-object shape everywhere. The blob appears only in the wash, from four frozen SVG paths in `components/Blob.tsx`, generated once by a seeded Catmull-Rom through jittered radial points and then pasted in. Nothing is computed at runtime.

Lines are hairlines: 1px on every glass edge, 1.5px on the menu keycap, the flow index badge, the timeline's live ring and the live-link's gradient ring, 2px on the card gradient edge, the timeline rail, the flow connector rule and the hero wave, 2.5px on the timeline node, 1px **dashed** on the flow bus and, since 2026-09-10, on an **incoming** timeline role's own node and card border — the same "not quite a solid thing yet" reading the flow bus already carries, extended to an employment row that is agreed but not started. Icon strokes are 2 to 3px.

Icons are authored, not imported: `components/Icon.tsx` exports ArrowRight, ArrowLeft and ArrowUpRight on a 20×20 viewBox at `stroke-width: 2.25`, round caps and joins, `currentColor`, `fill: none`. The shell's three-rule menu glyph, the theme toggle's sun and moon (2) and the monogram (2.4) match that construction. `ArrowDown` was retired 2026-09-10 with its one caller: the hero's fold cue moved to a component-local `ScrollCue` (see Layout) that is not shared, because it is a signature mark for that one spot rather than a grammar icon. Project marks in `components/Artifact.tsx` are the same language at 2.5 stroke: geometric diagrams, never illustration. They render inside the card media frame and, at 44px, inside the timeline's project chip.
...
- **Background:** `{colors.glass}` at rest, `{colors.glass-strong}` on hover, no blur; the tint alone carries legibility (The Tint-Is-Legibility Rule)
- **Border:** 1px `{colors.glass-edge}`; on hover it mixes 55% signal into the edge
- **Shadow Strategy:** none; see Elevation & Depth
- **Internal Padding:** 30px (18px under 760px); the flagship runs 40px (20px under 760px)
- **Layout** (reworked 2026-09-03): `.card` is a grid of `minmax(0, 1fr) minmax(0, 38%)` columns and `auto 1fr auto` rows, 24px column gap, 14px row gap. `.card__head` (name, and the organisation where one is printed) spans **both** columns in row 1; `.card__body` (tagline, fact, use: a flex column at a 14px gap) sits in column 1 row 2; `.card__foot` in column 1 row 3; `.card__media` in column 2 spanning rows 2 to 4, `align-self: center`. The `1fr` body row is what pushes the foot to the bottom edge. The head spans because, sharing the text column with the mark, a two-word name like "Santa Claws" broke one word per line in any narrow card. The flagship is `minmax(0, 1.15fr) minmax(0, 1fr)` columns and `1fr auto auto auto 1fr` rows with a zero row gap, 32px column gap, `min-height: 400px`: head in row 2, body in row 3 (which reinstates the 14px as `padding-top`), foot in row 4, media spanning every row in column 2 in a 4/3 box, so the flexible outer rows centre the text group against the taller mark (`align-items: center` alone cannot, because a spanning mark hands its extra height to every auto row it crosses).

  **Under 760px, reworked again (2026-09-05):** a standard card becomes a header-row grid (`grid-template-columns: 60px minmax(0, 1fr)`, 14px column gap, 12px row gap, `--card-pad: 18px`): the mark shrinks to a **60px chip** beside the name (row 1), the tagline runs the full card width below it (row 2, `.card__body { grid-column: 1 / -1 }`), and the foot spans full width in row 3. The old 34%-column mark layout gave the text column only 183px on a 390-wide phone and ran taglines six lines deep (2026-09-05 measurement); a 60px chip beside a full-width tagline was the fix. The chip takes `--radius` (16px, the same radius the timeline's project mark uses) rather than `--radius-card`: 22px on a 60px square reads as a circle. Fine detail inside a mark (the lobster's eyes, legs, pom-pom) is hidden at chip size (`.card__art .mark__fine { display: none }`), since at 42px of art the silhouette carries the mark. The repo control becomes a uniform 52px corner circle on every card, ScorelyAI included (with the live pill beside it there is still exactly enough width: 16 + 52 + 12 + 150 + 12 + 52 + 16 = 310 of 351). The flagship stacks cover, head, body, foot in four rows and its mark becomes a **16:9 cover band**: full card width, `max-height: 176px`, `border-radius: var(--radius-card)`. On a phone the flagship's one picture is the first thing on the page rather than sitting beside the text.

  **A drawn corner-arrow cue** (`.card__cue`) appears only under `@media (hover: none)`: a 52px glass-raised circle at the card's bottom-right corner holding a 22px arrow, which nudges 3px on `.card:active`. It exists because the desktop affordance for "this card opens something" is the hover lift and glow, and a touch device has no hover state to supply that signal; the cue is hidden outright wherever `hover: hover` exists, so a mouse user never sees both.
...
- **Specular highlight** (`.card::after`): a `68% 78%` ellipse of `--card-glow` (white at 0.55 in light, 0.17 in dark) fading to transparent at 62%, fixed at `50% -8%` - above the card's top edge, which is where a light would be for something that has just risen toward the reader. It sits at `z-index: -1` inside the card's own stacking context (the card's `--lift`/`--press` transform makes it one, since nothing on the card blurs its backdrop any more), so it is above the frosted fill and below every child, including the full-card link overlay. Only its opacity transitions: 240ms in, 200ms out. It is a light, not a movement, so it stays under reduced motion. A gloss on the glass, never a colour.
...
- **Style:** `{colors.glass-raised}`, 1px `{colors.glass-edge}`, 999px, `padding: 6px 13px`, 0.8125rem at 500, line-height 1.2, `--ink`, `white-space: nowrap`, `width: fit-content`.
...
**The Arrow Grammar Rule**, in two cases. A **leading** arrow means the link **stays on the site**: `← back to the work` (which points back, and whose hover moves it left rather than right). A **trailing `ArrowUpRight`** means the link **leaves**: `Try ScorelyAI live ↗`, `{name} on GitHub ↗` in the case-study foot, and `Résumé ↗` (opened as a PDF in a new tab, so it follows "leaves" rather than carrying a third, download-specific arrow). That distinction is what makes an on-site link and an off-site link read as different promises rather than as two links of equal weight. The card's own case-study link carries no arrow and no label at all (see below); the whole card is the promise instead of a line of type naming it.
...
- **Contact email**: display face, `clamp(24px, 4.6vw, 28px)` at 600, in a target at least 44px tall (padding, never font size), underlined with a 3px coral rule at 10px offset. Hover swaps the underline to ink; the text colour never moves. The floor is 24px because the loudest thing on the page has to be loudest on a phone too: it used to resolve to 16px at 390, smaller than the body copy beside it. Under 360px it steps down one size, the one place the email named breakpoint below The 1140 Rule's tier does anything.
- **Copy Email** (`components/CopyEmail.tsx`, `.copy`): a small icon button beside the address, added because a phone with no mail client configured opens nothing on `mailto:` and a recruiter should not have to retype an address by hand. `navigator.clipboard.writeText` on click, an authored copy/check icon pair that cross-fades on `[data-done]`, and a `sr-only` `aria-live="polite"` region that fills with "Copied" for 1.6 seconds and clears itself, so a screen-reader user gets the same confirmation a sighted one reads off the icon swap. If the clipboard permission is denied, it fails silently: the address is still selectable text beside it.
- **Card link** (`.card__hit`): renders no visible text, arrow or pill, only an sr-only accessible name (`View the {name} case study`) for keyboard and screen-reader users. Removed 2026-09-03 (Parth: every card already glows and lifts on hover, so a labelled button beside that read as clutter). The anchor itself owns the full-card overlay (see The Card-As-Link Rule) and collapses to a 0×0 box around its sr-only child, so it never occupies foot layout. `.card:focus-within` gives keyboard focus the same glow `.card:hover` gives a pointer, so tabbing onto the invisible link still answers. **Repo button** (`.card__repo`): a 56px circle (up from 40 on 2026-09-03, Parth: "a lot bigger"; it clears the 44px touch size on every device, so the old `pointer: coarse` override is gone), and 64px with a 32px mark in the corner variant every card except ScorelyAI wears (see Foot above: ScorelyAI's foot leads with the live pill, and the two controls share a baseline there), wearing the plain raised glass every small surface wears - 1px `--glass-edge`, `--glass-raised` - so the foot has one material for *a control that leaves this page*, at two weights: the gradient-ringed pill says use this, the plain circle says read the source. It holds a 28px GitHub mark in `--ink-2` and no label; the accessible name is on `aria-label`. Hover tints the edge 55% toward `--gh-hue-1` and lifts 2px (movement only, so `prefers-reduced-motion` keeps the colour and drops the rise); press composes `scale(0.97)` on top of the lift.
...
- **Shell mark** (`.shell__mark`): a 58px-tall glass pill (50px under 760px), `{colors.glass-strong}`, 1px edge, `padding: 0 16px 0 10px`, holding the 62px-wide PD monogram (50px under 760px) and, 4px to its right, a 20px three-rule glyph in a 24px box (`.shell__mark-lines`, 2.25 stroke, round caps, the third rule shorter). It is the `SheetTrigger`. Hover moves it to `{colors.glass-raised}` and tints the edge 40% signal; press scales 0.97.
- **Theme toggle** (`.theme-toggle`): a 44px glass circle on the same fill and edge, holding an 18px sun or moon that cross-fades (and, with motion allowed, un-rotates from −45° / 0.9 scale) over 200ms. Same hover and press as the mark. `ThemeProvider` (`next-themes`, `attribute="class"`) defaults to `defaultTheme="system"` with `enableSystem`, on phone and desktop alike, so a cold visit follows the OS rather than opening light and waiting for a click; a `system-theme` capture mode in the mobile harness asserts this on every route in both color schemes. On every mount and every toggle, `ThemeToggle` reads the computed `--ground` back off `document.documentElement` and writes it onto **both** `meta[name="theme-color"]` tags (`app/layout.tsx`'s `viewport.themeColor` array, one entry per `prefers-color-scheme`), deferred one `requestAnimationFrame` because `next-themes` commits its own class-writing effect after this one and the value has to be read post-paint. Without the defer, a manual toggle away from the OS scheme left the phone's browser chrome on the wrong colour. The CSS owns the colour; the effect only ever reads it back, never repeats the hex.
- **The menu sheet** opens from the **left** on desktop, the mark's own edge, so it scales in from its trigger rather than from the far side of the screen. A shadcn/Radix `Sheet` (`side="left"`) styled `.menu`: 86% ground, a 1px `--glass-edge` **right** border, 48px/40px padding (40px/24px and 88% wide under 400px), 40px gap. Links are display-face uppercase `clamp(1.25rem, 10cqw, 2.5rem)` at 0.01em with a bordered keycap on the right (0.75rem / 600 / 0.08em, 1.5px `currentColor`, 6px radius, `2px 6px`, `--ink-2`): **four: 1 Work, 2 Experience, 3 About, 4 Contact**. The label carries `min-width: 0`, `overflow-wrap: break-word` (never `anywhere`, which would zero its min-content width and let it break mid-word with room to spare) and the keycap `flex: none`, so the keycaps are a column at every width and the longest label still sets on one line: measured 23.3px at 320, 29.4 at 390 and 30.3 at 1440, EXPERIENCE on a single line at each. Each link rises in on open at 420ms with a `calc(var(--i) * 40ms + 80ms)` stagger; under reduced motion the sheet fades (200ms in, 140ms out) instead of sliding. Hover turns a link `--signal-ink`; press scales 0.98. There is no visible close button on desktop - Escape, the overlay and the mark all close it, and the shadcn default was the one control on the site drawn in another vocabulary. The foot lists email, GitHub, LinkedIn and the résumé at 0.9375rem in `--ink-2`.

**The Bottom Sheet Rule.** Under 760px the same Radix sheet becomes a bottom sheet by geometry alone, not a different component: `.menu[data-side="left"]` gets `inset: auto 0 0 0`, full width, `height: auto`, `max-height: min(88svh, 640px)`, a top border instead of a right one, and `border-radius` rounding only its top two corners, because a phone's thumb lives at the bottom of the screen and a panel rising from under it is one motion away from where the hand already is. Desktop's left drawer is untouched; the shadcn/Tailwind utility classes for the left slide sit in Tailwind's utilities layer, so these unlayered phone rules beat them by cascade order regardless of specificity. Four things change with the geometry:
  - **A grabber** (`.menu__grab`, phones only): a 36×4px rounded bar, `color-mix(in oklab, var(--ink) 22%, transparent)`, centred at the top of the sheet, the conventional affordance for "this panel drags."
  - **A `Close menu` button** (`.menu__close`, phones only, `showCloseButton={false}` on the shadcn `SheetContent` so this is the only close control drawn): a 44px glass-raised circle top-right holding the authored cross-stroke icon, `aria-label="Close menu"`, press scales 0.97. It sits after the nav in DOM order, so Tab reaches the section links first, and is positioned top-right by CSS.
  - **The email link moves first** in `.menu__foot`, ahead of the GitHub/LinkedIn/résumé row, in the same display-face-plus-coral-rule treatment Contact uses: the thumb-reachable way to email from anywhere on the site, and the loudest thing in the sheet's foot the way it is the loudest thing in Contact.
  - **The three foot pills wrap.** `.menu__foot-row` takes `flex-wrap: wrap` under 760px; each pill (GitHub, LinkedIn, résumé) is `flex: 1 1 0`, 44px tall, glass-raised on the sheet's own glass. Before this the row had no wrap rule, and at 200% text the three pills overflowed the sheet width, a real bug found and fixed in this program (PR 11). At normal text the three still sit in one row; wrap only engages when they need it.
  
  The number-shortcut keycaps are hidden from touch entirely (`@media (pointer: coarse) { .menu__key { display: none } }`, "a keycap on a touch screen is a promise the device cannot keep"), though the shortcuts themselves stay live while the sheet is open regardless of pointer type; see The number shortcuts, below.
...
The availability pill under the hero actions: `{colors.glass-raised}`, 1px edge, 999px, `padding: 9px 16px 9px 14px`, 0.875rem / 500 / 0.06em uppercase in `--ink-2`, with a 9px coral dot 10px to the left. The dot pulses: a `::after` ring scaling 1 → 3 and fading 0.5 → 0 over 2.6s on `--ease-out`, transform and opacity only, never an animated box-shadow. It is a pill because it sits low in the first viewport, outside the calm band, where no ink was dark enough in both themes; see The Surface-Not-Spacing Rule.

### Signature Component: the Experience timeline

One rail, a node per role, the period in display type, a glass card per role that ends in the project it produced. Work says what was built; this says who paid for it, and every card with a project links back to the card it grew out of, with that project's own mark, so a reader walks between the two without hunting. An **incoming** role (agreed, not started: `incoming: true` in `data/experience.ts`, since 2026-09-03) renders the role, the organisation and its season (FALL 2026) in the period slot and nothing else: no ownership sentence, no project chip, and the plain ink node rather than the live one, because live means happening now. Since 2026-09-10 its node and card border also switch to `border-style: dashed` (`data-incoming` on `.timeline__item`): the two data-rich rows below it stay solid, so the row with nothing else to show reads as a different *kind* of fact — agreed, not yet real — rather than a thinner version of the same one, the same outline-vs-fill grammar the hero draws between "HEY, I'M" and the filled name, and the same "not quite a solid thing yet" the flow bus's dashed edge already carries. Nothing is inferred to fill the gaps. Every card leads with the **position** and puts the organisation on the line under it (since 2026-09-03, at Parth's request), because a recruiter scans for the title first; the two are one unit at the card's 6px gap. The date range is the sequence signal; there are no 01/02/03 numbers, and the role column carries no role ("Internship" is an employment type, not a title). Returns `null` when the list is empty.
...
- **Node** (`.timeline__node`): a 14px circle on the rail, 2.5px `--ink` border, `--ground` fill so the rail visibly passes behind it, `translate: -50% 0`. The ongoing role (`data-live`) takes a `--signal` border and a static 1.5px ring 6px outside it at 0.45 opacity. Static, because the pulse belongs to exactly one element and that is the hero's dot. The incoming role (`data-incoming`) keeps the plain ink border but switches it to dashed.
- **Card** (`.timeline__card`): `{colors.glass}`, 1px edge, 22px, `padding: 26px 28px 28px`, a flex column with a 6px gap: role (`.timeline__role`, 1.0625rem / 600), organisation (`.timeline__org`, 1rem, `--ink-2`), ownership (1rem / 1.55, `--ink-2`, 62ch, 8px above), then the project chip 18px below. Dashed on an incoming role (`data-incoming`), same edge colour and width, no new opacity.
...
- **Node** (`.flow__node`): `{colors.glass}`, 1px edge, 22px, `padding: 18px 18px 20px`, a 6px-gap column: a 26px index badge (1.5px `currentColor` circle, display face 0.75rem / 700, `--ink-2`, `aria-hidden`), the title (1rem / 600 / 1.3), the detail (0.875rem / 1.45, `--ink-2`).
- **Node hover:** the border warms to 40% signal over 200ms and the same specular highlight the cards carry comes up: `.flow__node::after` is a 300px circle of `--card-glow` fading to transparent at 62%, centred on `--mx`/`--my`, at `z-index: -1` inside the node's own stacking context, `.flow__node` carries no transform of its own, unlike `.card` and `.live-link`, so it declares `isolation: isolate` explicitly to keep the highlight from escaping to the document root, opacity-only, 320ms in and 200ms out. The nodes are the same glass the cards are, so the case study answers the pointer with one material. **No tilt**, and that is the reason the flow uses a different component from the card: the connectors are pseudo-elements aligned to the node edges, so rotating a node in 3D would pull its edge off the arrow pointing at it. Like the card highlight it is a light rather than a movement, so reduced motion keeps it.
...
- **Bus** (`.flow__bus`): 16px under the row, `{colors.glass}`, **1px dashed** `--ink` at 32% (dashed so it reads as a substrate rather than a sixth node), `--radius` (1rem), `padding: 16px 20px`, a `max-content minmax(0, 1fr)` grid with a `4px 22px` gap: title 0.9375rem / 600 in `--ink`, detail 0.875rem / 1.5 in `--ink-2`. One column under 900px.
...
- **Screens** (`.shots`): the slideshow, last block before the foot, `padding: 64px 0 0` on the same rhythm as a section, under a "Screens" heading at the case-section size. With no media it does not render at all; with one item it renders the frame and caption and no controls.
  ...
  - **Phone** (≤760px): `--shots-h` is `calc((100vw - var(--gutter) * 2) / 1.6)` and a tall capture (`[data-tall]`) fills the column and shows its top (`object-fit: cover; object-position: top`). **The one place this set is cropped**, because fitted whole into a 351px column a full-page capture is a 122px-wide thumbnail. Desktop still fits it whole and crops nothing. The crop is recoverable: a tall slide is a button that opens a native `<dialog>` (`showModal()`) holding the full, un-cropped, pinch-zoomable screenshot in a scrollable frame, with its own close control and a click on the backdrop (`e.target === e.currentTarget`) to dismiss. Entrance and exit use `@starting-style` and `allow-discrete` rather than a JS-driven open/close animation, so the dialog's own top-layer promotion carries the transition.
...
- **Off-screen and hidden work is paused:** the roll checks `document.hidden`, an IntersectionObserver and pointer hover before advancing; the scroll ring and `ScrollScrub.tsx` read position inside a single rAF on a passive listener; Reveal only hides elements that are below the viewport at load, so nothing flashes; the spotlight's loop runs only while its spring is moving (rest is 0.15px and 2px/s), clamps `dt` to 32ms so a waking tab does not fling the light across the screen, and hides on window blur, on a hidden tab, and when the pointer leaves the window. `Reveal.tsx`'s `IntersectionObserver` fires at `threshold: 0.12` with `rootMargin` tightened on a phone to `"0px 0px -8% 0px"` (against `"0px"` above 760px), so a below-the-fold block on a short phone viewport reveals slightly before it is fully in view rather than waiting for the same margin a tall desktop viewport can afford.
...
- **Ambient drift:** four blobs on 34s / 41s / 37s / 45s `ease-in-out infinite alternate` loops, each translating 5 to 7vmax and rotating 7 to 11°. Disabled outright under 760px (`.wash__blob { animation: none !important }`, four animated blobs is not a mobile budget) and under reduced motion. **This is a measured decision, not a permanent ban on phone wash motion.** The mobile-motion pass (PR 09) evaluated one candidate for phone motion beyond feedback and rises, a scroll-linked wash move, the two bottom blobs drifting up and the two top ones down on `animation-timeline: scroll()`, and wrote its own gate: ship it only behind a real-device 60fps measurement (Safari Web Inspector, Timelines, scrolling the Work grid) that this development environment cannot perform. It therefore ships frozen, not because scroll-linked wash motion is refused, but because it is unmeasured; the next session with a physical device can add it behind that same gate.
...
- **The flagship unveil** (`card-unveil`, on `.work__item--flagship .card` over `entry 0%` to `entry 100%`): a `clip-path: inset(0 0 100% 0 round var(--radius-card))` opens to `inset(0 0 0 0 round var(--radius-card))`, lagging so that halfway into the viewport only a fifth of the card is drawn (`inset(0 0 80% 0)` at 50%) and the clip edge closes the gap over the second half, so the card is seen being drawn rather than merely arriving; `scale:` settles 0.96 → 1 and opacity rises 0.5 → 0.85 → 1. Inside the guard the flagship's `Reveal` wrapper is neutralised (`opacity: 1; transform: none; transition: none`), because two entrances on one element is one too many; outside it the Reveal rise remains the entrance, so nothing is ever hidden without something to show it. The card's own `transform` (lift, press) is untouched and composes.
  ...
  **`card-unveil` is switched off entirely under 760px** (`animation: none`), and it has been evaluated twice, not assumed off by default. First, at the hero's phone rework (PR 02): the flagship card is already a fifth to a quarter of the way into the viewport at load by design (the fold peek, see Layout), so the unveil's clip-path would start already clipped and half-transparent instead of running its intended arc. The hero's `recede` handoff is the phone's whole scroll handoff on its own. Second, PR 09's motion pass reconsidered it directly and kept it off: unveiling a *second* card (the flagship is no longer first in view once the fold peek is in place) would be decoration seen once, against a phone motion budget that asks for quiet ambience, not a second scroll-linked entrance to add.
...
- **Media motion collapses under reduced motion and below 761px; the handoff collapses under reduced motion only.** Below 761px the recede runs without its blur (`--recede-blur: 0px`, against 10px above 761px) and, since PR 09, with a gentler arc of its own: `--recede-scale: 0.94` and `--recede-shift: 8%`, against 0.9 and 22% on desktop. The desktop 22% shift (about 146px) would push the fading name into the WORK label now that the hero is bottom-set and the flagship card is peeking into the fold; a 39px phone name shrinking all the way to the desktop's 90% reads as a wobble at that scale, where 94% reads as depth instead. The flagship's `card-unveil` is off outright below 760px (see above), not merely unchanged. Mobile gets less by rule, not by accident.
...
- **No `backdrop-filter` on anything that travels, retired 2026-09-05, vacuously.** The constraint used to bar the case-study header panel from moving under scroll unless it were opaque, because a moving `backdrop-filter` element re-samples and re-blurs its backdrop every frame over the blob wash and the grain layer. It no longer distinguishes anything: no surface anywhere carries `backdrop-filter` to begin with (see The Tint-Is-Legibility Rule). The recede still runs on the bare hero and on the opaque `--media-well` headline panel, but for a simpler reason now, `filter: blur(var(--recede-blur))`, applied to the element's own pixels and unrelated to the glass system, stays cheap regardless of what sits behind it. The unveil's `scale:` 0.96 → 1 is the one scroll-driven change on a frosted surface; it is a 4% settle inside a clip, and the edge the eye follows is the clip, not a travelling pane. See The Opaque-Media Rule for why the media well stays opaque anyway.
...
- **Do** build every new surface from the glass ladder: `--glass` for a panel, `--glass-strong` for a lit or heavy one, `--glass-raised` for a pill or control sitting on glass, always with a 1px `--glass-edge`. No `backdrop-filter`: the tint alone carries legibility (The Tint-Is-Legibility Rule).
...
- **Do** key every phone rule off **760px**, the one phone breakpoint. Below it, reach for 400px or 360px only if the existing named exception at that width already covers the case; a third named exception needs a real reason.
- **Do** route every fixed-edge phone value through the `--safe-*` tokens (`env(safe-area-inset-*, 0px)`) rather than a bare pixel value, and keep `viewport.viewportFit: "cover"` in `app/layout.tsx`.
- **Do** give every new glass surface a `prefers-reduced-transparency: reduce` fallback to `--glass-solid`, and give any new sequence-bearing gradient (a rail, a connector, anything the reader has to read in order) a `forced-colors: active` fallback to `CanvasText`.
- **Do** give each `content-visibility: auto` selector its own `contain-intrinsic-size` guess, sized to what that section actually renders; a shared guess undershoots most sections and breaks fragment-navigation landing on the ones it undershoots worst (The Per-Selector Intrinsic Size Rule).
- **Do** regenerate mobile screenshots and contrast numbers with `node scripts/mobile-capture.mjs` and `node scripts/mobile-contrast.mjs` (both need `playwright-core` installed with `--no-save` in the current worktree, and a `next build && next start` running) after any visual change, rather than hand-describing what a phone viewport now looks like.

### Don't:

- **Don't** add a `box-shadow`. This system draws exactly one, `--shadow-lift`, in exactly one state (a card hovered off the page); at rest and everywhere else there are none; elevation is an edge, never a shadow (The Edge-Not-Shadow Rule).
...
- **Don't** reintroduce a 720px or 640px breakpoint. Everything that used to key off either now keys off 760px; a phone is one device class with one threshold (The One Phone Breakpoint Rule).
- **Don't** treat the frozen phone wash or the disabled flagship `card-unveil` as permanent style rules. Both are measured decisions with a documented reason (an unmeasured real-device frame-rate gate for the wash; a fold-peek conflict and a "seen once" call for the unveil), not prohibitions on phone scroll motion in general. A future session with a physical device and a fresh measurement can revisit either.
- **Don't** give a `content-visibility: auto` section a shared blanket `contain-intrinsic-size`; size each selector to its own content (The Per-Selector Intrinsic Size Rule).
- **Don't** hardcode a `mailto:` link as the only way to get in touch on a phone; a copy-to-clipboard control belongs beside it, because a phone with no configured mail client opens nothing.
