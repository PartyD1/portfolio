...
**The Arrow Grammar Rule**, in three cases. A **leading** arrow means the link **stays on the site**: `→ see my work`, `→ more about me`, `← back to the work` (which points back, and whose hover moves it left rather than right). A **trailing `ArrowUpRight`** means the link **leaves**: `Try ScorelyAI live ↗`, `{name} on GitHub ↗` in the case-study foot. A **trailing `ArrowDown`** means it downloads: `Résumé ↓`. That distinction is what makes an on-site link and an off-site link read as different promises rather than as two links of equal weight. The card's own case-study link carries no arrow and no label at all (see below); the whole card is the promise instead of a line of type naming it.
