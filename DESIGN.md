...
Work is a two-column grid (`repeat(2, minmax(0, 1fr))`, 18px gap) with the flagship spanning `1 / -1`; it collapses to one column at **1000px**, not 760 (since 2026-09-03). Past four non-flagship cards, a full-span `.work__break` (28px, 16px under 1000px, 2026-09-10) lands after the first non-flagship pair: pure spacing, no border and no label, so the grid reads as two decisions (the flagship's row, then the rest) rather than one field of same-weight cards past the four-option working-memory ceiling. At three or fewer non-flagship cards nothing needs splitting and it does not render. Two 370px cards side by side gave each name and tagline a 155px column; a two-column card needs about 235px of text column to read, which is what 1000px provides. The phone-level card rules (the 60px mark chip, the flagship's 16:9 cover band, the corner cue) stay at 760px, so between the two a single card at full width uses the desktop card layout. `.work__item` is itself `display: grid` so `.card { height: 100% }` resolves and a row's cards share a bottom edge. Experience is a rail-and-column timeline (below). About is `minmax(0, 1fr) minmax(0, 1.5fr)` with a 48px gap, collapsing to one column with a 20px gap at 760px. Contact is left-aligned like everything else, with 96px/128px vertical padding.
