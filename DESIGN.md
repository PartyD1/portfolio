...
Three of these stops form the world's one gradient line, `lilac → apricot → sky` (`--blob-c-1`, `--blob-b-1`, `--blob-a-2`). It is painted exactly three times: at 120° as the 2px card edge, top-to-bottom as the 2px timeline rail, and bottom-left to top-right as the fill of a hovered GitHub mark.

The mark is the one of the three that does **not** use the blob values directly, and the reason is a rule about where those values come from. The blob tokens are the wash's colours *seen through glass*; painted straight onto white glass at 19px they come out paler than the `--ink-2` they replace, so hover made the mark quieter instead of louder. Mixing them toward `--ink` restored the weight and turned apricot into mud - the same failure `--blob-b-2` and `--blob-d-3` hit in the dark theme. So light gets three hand-picked deeper cousins (`{colors.gh-hue-1}`, `{colors.gh-hue-2}`, `{colors.gh-hue-3}`) and dark, whose ground makes the pastels the brightest thing the button can hold, aliases the blob tokens unchanged. **Both themes define all three**: a value that exists in only one theme is a bug (see Colour, which opens on the two-token-set rule).
...
- **Label** (500, 0.875rem): captions, flow details, the footer and the mobile tech name, at normal tracking in sentence case. The tracked-caps 0.06em variant of this step belonged to the hero's status line and went with it; the two tracked-caps labels that remain (`.card__label`, `.pending-note`) sit smaller and at 0.08em.
...
**The Uppercase Display Rule.** `h1`, `h2` and `h3` are uppercase Unbounded at 700 with positive tracking (0.015em base). Body copy is never uppercase; only the two micro steps and the timeline period are, and they always carry at least 0.02em tracking (0.06em and up below 15px) to stay legible.

**The Tracked-Caps-In-The-Foot Rule.** The `.pill` carries no tracking and no uppercase by default. `.card__label` adds both, because a card foot is the only place this system allows tracked caps as a label. A pill anywhere else (About's range) is sentence case at 500.
...
**The Shared-Left-Edge Rule.** The hero headline, descriptor, facts line, actions and every section head below them sit on the same left edge, asserted to 0.0px at every width in both themes. This is what left-aligning the hero buys, and it is why Contact is not centred: with the hero left-aligned, a single centred block reads as an oversight rather than as a finale.
...
- **Foot** (`.card__foot`): a wrapping flex row, 12px/18px gaps, `margin-top: auto` so all feet in a row share a baseline, 18px above. Order: the live-link pill where a `demo` exists, then the `case study` arrow link, then the repo icon button. On a card carrying all three the button wraps to a second line at every width the grid produces; that is the foot working as designed, and it is why the control became a 40px circle rather than a wider labelled link.
- **Hover:** the card **rises off the page**. `--lift: -8px` over 240ms `--ease-out`, `{colors.glass-strong}`, an edge tinted to 55% signal, and `--shadow-lift` underneath. The case-study arrow nudges 4px from a hover anywhere on the card, 3px from a hover on its own label. Every hover rule sits behind `@media (hover: hover) and (pointer: fine)`, and the lift and its shadow sit inside a nested `prefers-reduced-motion: no-preference` as well: under `reduce` the card still takes the brighter glass, the signal edge and the light, it just does not travel.
...
**The Arrow Grammar Rule**, in three cases. A **leading** arrow means the link **stays on the site**: `→ see my work`, `→ more about me`, `→ case study`, `← back to the work` (which points back, and whose hover moves it left rather than right). A **trailing `ArrowUpRight`** means the link **leaves**: `Try ScorelyAI live ↗`, `{name} on GitHub ↗` in the case-study foot. A **trailing `ArrowDown`** means it downloads: `Résumé ↓`. That distinction is what makes `case study` and an off-site link read as different promises rather than as two links of equal weight.
...
**The Drawn Underline Rule.** Every *unbounded* arrow link (`.link-arrow`, `.case__back`) carries a `::after` rule, 2px tall, `--signal`, inset 4px from each end and 2px above the bottom, held at `scaleX(0)`. On hover it draws in from the left (`transform-origin: left`, 260ms) and on leave it exits to the right (`transform-origin: right`, 160ms): the origin flips with the state, so the line sweeps through rather than retreating the way it came. `transform` only, so it costs nothing, and the exit is faster than the entrance. It is `--signal` and not `--signal-ink` because it is a mark, not text (The Two Corals Rule). The arrow travels with it: 6px right on `.link-arrow`, 6px left on `.case__back`, the same 6px the timeline chip takes. The card's own hit label is the exception and is not an arrow link: it is a pill hit area inside a surface that is already moving, so its arrow nudges 4px from a hover on the card and 3px from a hover on the label.

**The rule is drawn only where the link has no box of its own.** The Experience timeline's project chip is a bordered glass pill, and a 2px rule sitting inside a rounded pill reads as a rendering slip rather than a hover state; it takes the coral border and the same 6px nudge instead. The menu links, the footer links and `.card__hit` do not draw the rule either: the menu link steps 8px toward the pointer (`--shift`), since it is seen only when the sheet is open and can afford to move, and the footer link and card hit both have a pill background to answer with.
...
- **Card link** (`.card__hit`): 15px/600 in a pill hit area created with negative margins (`padding: 8px 14px; margin: -8px -14px`), so the tap target is generous without changing layout. It also owns the full-card overlay; see The Card-As-Link Rule. **Repo button** (`.card__repo`): a 40px circle (44px under `pointer: coarse`, growing in both axes to stay round) wearing the live-link pill's raised glass - 1px `--glass-edge`, `--glass-raised`, `blur(12px)` - so the foot has one vocabulary for *a control that leaves this page*, at two weights: the pill says use this, the circle says read the source. It holds a 19px GitHub mark in `--ink-2` and no label; the accessible name is on `aria-label`. Hover tints the edge 55% toward `--gh-hue-1` and lifts 2px (movement only, so `prefers-reduced-motion` keeps the colour and drops the rise); press composes `scale(0.97)` on top of the lift.
...
- **Staggers stay 30 to 80ms** per step: hero 60/120/180ms across name → descriptor → facts → actions, cards 50ms, timeline items 70ms, flow nodes 60ms, menu links 40ms. Longer reads as the page being slow.
...
- **Do** drive any new state on `.card` through `--lift` or `--press`, and any new state on the menu link or the live-link through their own `--shift` / `--lift` / `--press`, rather than a second `transform` declaration (The One Composed Transform Rule).
...
- **Don't** draw the arrow-link underline on a link that already has a box: a 2px rule inside a pill reads as a rendering slip, and the timeline chip, the footer link and `.card__hit` answer with a border or a background instead (The Drawn Underline Rule).
