...
| P2 | Wave Function Collapse and Pewter Platformer, which have no `study` at all today | **In scope.** Both get a diagram built from scratch, not left absent. Each is gated on a short mechanism interview with Parth before its PR is built (see PR 06 and PR 07) — neither mechanism is documented precisely enough in `PRODUCT.md` or `data/projects.ts` to draw without inventing detail, and "never invent" outranks "always ship a diagram." |
| P3 | Diagram vocabulary | **The mechanism picks its shape.** No single diagram component is forced onto all seven projects. `Flow`/`Flow.tsx` (the rail-and-node stepper) stays for the projects it actually fits; projects whose real mechanism is a loop, a tree, or a shared-store/hub-and-spoke topology get a diagram built for that shape instead of being bent into a stepper. See "Diagram vocabulary" below. |
...
| Wave Function Collapse | **None** | none (deliberately: the project's own README says the collapse/propagation logic is "still to build") | Unconfirmed. The textbook algorithm is grid-based constraint propagation; whether the shipped code implements that yet is exactly what PR 06's interview needs to settle before any diagram is drawn. |
...
Two things every audit row agrees on: nothing here is a small trim. Operations Agent and Santa Claws already have a diagram that has to change shape, not just lose its neighboring prose. WordPlay's stepper is quietly two mechanisms braided into one list. And two projects need a diagram invented before anything can be removed, because there is nothing to lose yet.

## Diagram vocabulary

Five shapes cover all seven projects. `Flow` (rail-and-node stepper, shipped 2026-09 in the mobile overhaul as PR 07) is one of the five, not the only one.
...
| **Grid/lattice** (new) | A 2D field where a decision at one cell propagates to its neighbors. The one shape with no precedent in the current component set — this is genuinely new visual work, not a variant of an existing primitive. | Wave Function Collapse, if the interview confirms the shipped code does propagation (not confirmed yet) |
...
1. **Never invent a mechanism.** A diagram is not a license to be more confident about how something works than the prose it replaces was. Where the real mechanism is unconfirmed (Wave Function Collapse, Pewter Platformer, and any fact flagged above as "an open call"), the PR is gated on confirming it with Parth first, not on a plausible guess.
2. **Blocked content ships by absence.** A project with no confirmed mechanism yet keeps `study` absent — same rule the site already lives by for tech lists, dates, and metrics. No placeholder diagram, no "how it works" skeleton.
3. **Same visual world.** New diagram shapes (hub-and-spoke, loop, tree, grid) use the existing token set — the gradient rail vocabulary, glass surfaces, `--ease-out`, both themes — not a new visual language. `DESIGN.md`'s component vocabulary gets new entries, not a fork of it.
...
- **Wave Function Collapse's actual implementation state** — PR 06 cannot start until this is confirmed. The repo's own README currently disclaims the core algorithm as unbuilt; the diagram must match whatever is true when PR 06 actually starts, which may be later than 2026-09-07.
...
- **Whether the grid/lattice shape (Wave Function Collapse) is worth a bespoke component for one project**, versus finding it can be expressed as a variant of the loop or stepper shape once the real mechanism is known. Decide this at PR 06, not now.
