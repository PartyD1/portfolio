...
Branch: `diagrams/08-records`. Size: S. Motion skill: no. `/polish`: no (no UI change). Depends on: PR 01 through 07 merged (06 and 07 may have closed with no diff — that is a valid merged state for this PR's purposes, not a blocker).
...
2. **Regenerate `DESIGN.md`** via the impeccable `document` command, so its component vocabulary section gains `Tree`, `Loop`, `HubAndSpoke`, and `Grid` (if built) alongside the existing `Flow` entry, and so it reflects a site where case studies carry no prose sections.
...
6. **Record the outcome of PR 06 and PR 07's interviews**, whichever way they resolved (diagram shipped, or `study` still absent), so a future session reading this plan folder does not re-run an interview that already happened.
