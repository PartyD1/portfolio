...
FIRST VIEWPORT: **Left-aligned hero, three display lines.** Line 1 `HEY, I'M` outline + `PARTH DOSHI` filled over a two-pass sine wave. Line 2 the outlined lead `AND I'M`, alone — an orphaned outlined lead-in reads as composition, which is what left-alignment buys. Line 3 the typing slot alone, a fixed-width box sized to the widest phrase so the caret never moves the layout. Content sits on the 1140 column and **shares its left edge with every section below it**. Then one-sentence subline; the availability subline when its fields are supplied; two arrow links (see my work / more about me); status line. **The status line must be visible in the fold at 1440×900 and 390×844, both themes.** Monogram top-left, theme toggle and dot-grid menu top-right, blobs in all four corners with a calm centre.

The typing slot never wraps at any width. The checkable invariant: `15.980em × font-size ≤ 0.95 × min(viewport − 2·gutter, 1140px)`, asserted at every breakpoint edge in both themes and again under a 20px root font. Both display clamps are px-bounded — a root-relative bound inside a vw-measured invariant is a silent-overflow generator.
