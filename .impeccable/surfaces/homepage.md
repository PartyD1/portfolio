...
FORM: User-pinned reference world (beats the roll) — Sharlee leads, Ram donated the status line (retired 2026-09-03; the reference still stands, the borrowing does not), Seán's structure survives in the section rhythm. Supersedes seed b925b460 and the editorial-grotesk pass. Phones (PR 04 of the mobile overhaul): the menu becomes a bottom sheet — a grabber, a close pill, the sheet's own foot row with the email first — replacing the desktop drawer by CSS at 760px, same `Shell.tsx` component underneath.
