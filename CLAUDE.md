...
**Rendering.** Everything is a server component except `components/Shell.tsx` (the PD-mark menu sheet plus the `1`-`4` keyboard shortcuts, which cross routes: they scroll when the section is in this document and `router.push('/#id')` when it is not), `components/Reveal.tsx` (scroll-in rise), `components/ThemeProvider.tsx` / `components/ThemeToggle.tsx`, `components/RoleRoll.tsx` and `components/ScrollRing.tsx`. `app/page.tsx` stacks `Hero → Work → Experience → About → Contact`; `app/work/[slug]/page.tsx` generates the seven case studies via `generateStaticParams`; `app/layout.tsx` mounts the fixed gradient `Wash`, the `Shell`, and the `Footer` around it and loads Unbounded (`--font-display`) and Hanken Grotesk (`--font-body`) via `next/font/google`.
