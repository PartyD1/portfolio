...
**Rendering.** Everything is a server component except `components/Shell.tsx` (the PD-mark menu sheet plus the `1`-`4` keyboard shortcuts, which cross routes: they scroll when the section is in this document and `router.push('/#id')` when it is not), `components/Reveal.tsx` (scroll-in rise), `components/ThemeProvider.tsx` / `components/ThemeToggle.tsx`, `components/RoleRoll.tsx`, `components/ScrollRing.tsx`, `components/Spotlight.tsx` (the pointer-following glow in the wash, a spring in rAF that stops at rest), `components/TiltCard.tsx` (the card's `<article>`, writing `--rx/--ry/--mx/--my` from the pointer; CSS does the motion), `components/FlowGlow.tsx` (the case-study diagram's `<ol>`, one delegated pointer listener placing `--mx/--my` on the hovered `.flow__node`) and `components/ScrollScrub.tsx` (WAAPI fallback for the `recede` scroll animation where `animation-timeline` is missing). `app/page.tsx` stacks `Hero → Work → Experience → About → Contact`; `app/work/[slug]/page.tsx` generates the seven case studies via `generateStaticParams`; `app/layout.tsx` mounts the fixed gradient `Wash`, the `Shell`, and the `Footer` around it and loads Unbounded (`--font-display`) and Hanken Grotesk (`--font-body`) via `next/font/google`.
...
**Styling.** All CSS lives in `app/globals.css`: tokens as custom properties on `:root`, redefined under `.dark` for the second theme, BEM-ish class names, one easing (`--ease-out`), one phone breakpoint at 760px (2026-09, mobile overhaul PR 01: gutter, sections, hero, blobs, drift, shell, cards, timeline, tech tiles, slideshow, case study, flow spacing — there is no 720 or 640 any more), plus two named exceptions below it, 400px (menu widens, pills wrap) and 360px (email steps down). **`DESIGN.md` is the written-down system** — palette roles, type ramp, spacing, motion grammar, component vocabulary, and the rules behind them (one ink, one accent, tracked caps only in a card foot, geometry not illustration, etc.). Read it before touching styles; it was generated from the built code and describes what actually ships. `.impeccable/design.json` is its machine-readable sidecar.
