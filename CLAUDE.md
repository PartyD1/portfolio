...
**Phones.** `scripts/mobile-capture.mjs` is the phone harness (committed; see its own header comment for every flag). From a worktree: `npm install --no-save --no-audit --no-fund playwright-core@1.62`, `npm run build && npx next start -p <a free port>` (3111 is not reserved for any one worktree; concurrent worktrees collide on it silently — `curl` still returns 200 from whichever process got there first, so confirm the `next start` log is clean before trusting a capture), then `node scripts/mobile-capture.mjs --port <that port> [flags]`. `scripts/mobile-contrast.mjs` re-measures contrast with the same composited-pixel method used for desktop. Every PR touching a phone surface must clear the shared gates in `.impeccable/plans/mobile-overhaul/SEQUENCE.md` section 5 (build, detector, overflow, targets, text floor, motion at rest, hover-gating, both themes, contrast, desktop-1440 parity); section 6 of the same file is the real-device checklist (Safari on an iPhone, Chrome on an Android) for the PRs that need eyes on hardware this environment doesn't have.

## Architecture

**Rendering.** Everything is a server component except `components/Shell.tsx` (the PD-mark menu sheet plus the `1`-`4` keyboard shortcuts, which cross routes: they scroll when the section is in this document and `router.push('/#id')` when it is not — the sheet itself is a left drawer above 760px and a bottom sheet under it, by CSS alone, same component), `components/Reveal.tsx` (scroll-in rise; `rootMargin` tuned tighter on phones), `components/ThemeProvider.tsx` / `components/ThemeToggle.tsx` (defaults to the OS theme; the toggle syncs both `theme-color` metas to the computed `--ground` on every mount and toggle, deferred a frame to survive a next-themes effect-ordering race), `components/RoleRoll.tsx`, `components/ScrollRing.tsx`, `components/Spotlight.tsx` (the pointer-following glow in the wash, a spring in rAF that stops at rest; bails on touch), `components/TiltCard.tsx` (the card's `<article>`, writing `--rx/--ry/--mx/--my` from the pointer; CSS does the motion), `components/FlowGlow.tsx` (the case-study diagram's `<ol>`, one delegated pointer listener placing `--mx/--my` on the hovered `.flow__node`; bails on touch), `components/ScrollScrub.tsx` (WAAPI fallback for the `recede` scroll animation where `animation-timeline` is missing), `components/Slideshow.tsx` (the peek/snap carousel and its tall-capture `<dialog>`) and `components/CopyEmail.tsx` (client, the one clipboard call, beside the address). `app/page.tsx` stacks `Hero → Work → Experience → About → Contact`; `app/work/[slug]/page.tsx` generates the six case studies via `generateStaticParams`; `app/layout.tsx` mounts the fixed gradient `Wash`, the `Shell`, and the `Footer` around it and loads Unbounded (`--font-display`) and Hanken Grotesk (`--font-body`) via `next/font/google`.

**Content is data, not JSX.** `data/projects.ts` is the single source for the six projects; `data/stack.ts` holds the tool groups (and is the ONE tool list — there is no `data/tech.ts`); `data/experience.ts` holds employment; `data/site.ts` holds links, the résumé flag and availability. `Work.tsx` maps projects into `ProjectCard.tsx`; a project's `slug` selects its geometric SVG mark in `components/Artifact.tsx` (unknown slug → no mark, silently). To add a project you need both a data entry and a mark; the route follows automatically.
...
**Styling.** All CSS lives in `app/globals.css`: tokens as custom properties on `:root`, redefined under `.dark` for the second theme, BEM-ish class names, one easing (`--ease-out`), one phone breakpoint at 760px (2026-09, mobile overhaul PR 01: gutter, sections, hero, blobs, drift, shell, cards, timeline, tech tiles, slideshow, case study, flow spacing — there is no 720 or 640 any more), plus two named exceptions below it, 400px (menu widens, pills wrap) and 360px (email steps down). Phone rules live in `@media (max-width: 760px)` blocks; a new phone rule joins one of them rather than opening a new one, and a new rule must be placed after every base rule it touches (equal specificity resolves by source order, not by media query — a rule added earlier in the file loses). **`DESIGN.md` is the written-down system** — palette roles, type ramp, spacing, motion grammar, component vocabulary, and the rules behind them (one ink, one accent, tracked caps only in a card foot, geometry not illustration, etc.). Read it before touching styles; it was generated from the built code and describes what actually ships. `.impeccable/design.json` is its machine-readable sidecar.

**Motion.** Every animation sits behind `@media (prefers-reduced-motion: no-preference)`. `Reveal` renders visible on the server and only hides elements that are below the viewport at load, so nothing flashes and reduced-motion users see the page complete. Phones: no ambient drift and no spotlight, by rule (the four wash blobs stop drifting entirely under 760px; `Spotlight`/`FlowGlow` already bail on touch); `:active { scale(0.97) }` press feedback on every pressable element; the full reduced-motion table lives in `DESIGN.md`, not here.
